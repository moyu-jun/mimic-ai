...
- ✅ **实机验证通过（含全屏游戏）**：
  1. 鼠标模拟页点「坐标拾取」→ 窗口隐藏 ✅
  2. 切到**全屏游戏**，点击目标位置左键 → 窗口恢复、坐标回填 ✅（方案 C 成功，全屏游戏内生效）
  3. 普通权限下同样生效 ✅
...
## 需求文档复核：对照最终实现重梳理 REQUIREMENTS

**完成时间**：2026-06-11

项目全部功能开发完成后，对照本 CHANGELOG 各阶段记录与实际代码，对 [REQUIREMENTS.md](./REQUIREMENTS.md) 做一次全面复核，修正过时描述、补全后期新增功能、消除与实现不符之处。

### 改动摘要

| 文件 | 改动类型 | 关键点 |
|------|---------|--------|
| [docs/REQUIREMENTS.md](./REQUIREMENTS.md) | 改 | 复核全文，修正/补全见下表 |

### 复核与修正点

| 章节 | 问题 | 修正 |
|------|------|------|
| § 3.3.1 首页 | 原描述「具体内容由设计文件细化」过于笼统 | 明确首页四块内容：版本说明、管理员状态+提权、驱动状态卡片（四态）、热键概览（核对 [HomePage.vue](../src/pages/HomePage.vue)） |
| § 3.3.2/3.3.3 | 间隔输入提交时机未写 | 补充「失焦或 Enter 提交，中间允许暂时为空」；按键页补充「重复 scanCode 显示 2 秒橙色提示」 |
| § 3.3.4 设置页 | 保存按钮职责与录音混淆（阶段 18.2 已归位） | 明确保存按钮仅负责热键持久化，提示音区块独立 |
| § 3.5 配置持久化 | — | 强调 `mimic.ini` 位于 **exe 同级目录**（核对 [config.rs](../src-tauri/src/config.rs) `ini_path()`）；补充运行期间写盘返回 "busy" 前端静默忽略 |
| § 3.6 全局热键 | 待机态表述不全 | 明确待机含 `Idle`/`Ready*`；状态不匹配时按键透传 |
| § 3.7 按键捕获 | 白名单外按键行为未写 | 补充「白名单外静默忽略，继续等待」 |
| § 3.8 坐标拾取 | 原描述「第一版可能不可用，全屏列入待确认」已过时 | 改为：基于 Interception 内核驱动，**支持独占全屏游戏**；命中左键透传 + `GetCursorPos` 读坐标；**无取消机制**；已知限制为锁定/隐藏光标的全屏游戏坐标可能不准（核对 [mouse_picker.rs](../src-tauri/src/mouse_picker.rs)） |
| § 3.9 模拟执行 | 无可执行动作行为未写 | 补充「无勾选/无有效坐标时热键被忽略，不进入运行态、不显示蒙版」 |
| § 3.10 运行状态 | 缺 `Recording` 态（阶段 18 新增） | 补入 `Recording` 行；补充状态切换规则（页面间直切、运行期禁止切页） |
| § 3.11 驱动管理 | 四态未明确 | 列明 `NotInstalled`/`InstalledNeedReboot`/`Ready`/`Error` 及 `Ready` 依赖 `create_context()` 验证 |
| § 3.13 热键提示音 | 触发语义不够精确 | 明确「仅状态真正切换才播放」「被忽略的热键不播放」「后触发优先打断」「文件缺失静默跳过」；声音文件为 exe 同级 `按键开启.wav`/`按键关闭.wav`（核对 [sound.rs](../src-tauri/src/sound.rs)） |
| § 3.14 提示音录制 | 阶段 18→18.2 多次迭代，原描述为旧的双面板方案 | 重写为统一面板五按钮流程（开始录制/结束录制/试听/保存/取消）；补充 5s 自停、最小选区 100ms、Web Audio 试听、`SND_PURGE` + `.tmp` 原子替换、离开设置页自动取消、`Recording` 状态隔离 |

### 关键决策

- **保留 REQ-CHANGE-001 变更记录**：该记录说明了热键支持范围扩展的来龙去脉，对理解修饰键支持有价值，故保留于文档顶部。
- **需求文档只描述功能行为**：复核中将混入的技术实现表述（具体 API、模块名）尽量收敛，技术细节归 DESIGN。坐标拾取/录制等处保留必要的「依赖 Interception 内核驱动」级别说明，因其直接影响功能边界（如能否在全屏游戏可用）。
- **标题栏按钮核实**：核对 [AppTitleBar.vue](../src/components/AppTitleBar.vue) 确认实际含「最小化 + 关闭」两个按钮，与 § 3.1 描述一致。

### 验证结果

- 逐条对照本 CHANGELOG 阶段 9（配置路径）、16.3（按需提权）、16.4/16.5（坐标拾取 Interception 方案）、17（提示音）、18/18.1/18.2（录制）与对应源码，确认 REQUIREMENTS 描述与最终实现一致。
- 全文扫描：已无 `%LOCALAPPDATA%`、`tauri-plugin-global-shortcut`、`WH_MOUSE_LL`、「待确认/第一版可能不可用」等过时或不确定措辞。
- 纯文档变更，不涉及代码，无需编译验证。

### 文档回写

- 本次仅修改 REQUIREMENTS.md；DESIGN.md / TASKS.md 未改动（其内容与最终实现已一致）。
